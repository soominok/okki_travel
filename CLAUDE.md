...
| 문서 | 언제 읽나 |
|---|---|
| `STATUS.md` | **세션 시작 시 항상.** 어디까지 왔고 다음에 뭘 하나 |
| `docs/ISSUES.md` | **막혔을 때 먼저.** 이미 밟은 지뢰 목록 |
...
| `docs/superpowers/specs/` | 수집 계층이 왜 이렇게 생겼는지 |
| `docs/JOURNAL.md` | "왜 이렇게 정했더라" 싶을 때 |
...
11. **예산 선점 없이 유료 소스를 호출하지 않는다.** `cost_per_call > 0`인 어댑터는
    `sources/budget.py`에서 크레딧을 확보한 뒤에만 부른다. 예산 부족은 실패가 아니라
    "이번엔 건너뜀"이다.
12. **fan-out을 어댑터 안에서 하지 않는다.** 기간을 쪼개는 것은 collector의 일이다.
    어댑터가 내부 루프를 돌면 호출 수가 숨어 예산을 강제할 수 없다.
13. **외부 서비스를 문서에 적기 전에 실제로 확인한다.** "무료 티어가 있다"는 기억은
    2026년 기준으로 자주 틀린다. 확인 안 했으면 "미확인"이라고 적는다. (`docs/ISSUES.md` I-002)
...
### 기록 — 작업 단위가 끝날 때마다 (건너뛰지 말 것)

이 프로젝트는 **오래 쉬었다 돌아와도 문서만 보고 재개할 수 있어야 한다.**
그래서 아래는 선택이 아니라 완료 조건의 일부다.

| 파일 | 언제 갱신 | 무엇을 |
|---|---|---|
| `STATUS.md` | **매 작업 단위 끝** | 완료 항목 체크, "지금 당장 할 일" 갱신, 새 블로커 |
| `docs/JOURNAL.md` | 세션 끝 또는 큰 결정 후 | **왜 그렇게 정했는지.** 대안을 왜 안 골랐는지까지 |
| `docs/ISSUES.md` | 문제를 해결한 직후 | 증상·원인·해결·**교훈**. 최신순으로 맨 위에 |

원칙:

- `STATUS.md`는 **현재 상태만** 담는다. 히스토리를 여기 쌓지 않는다 (그건 JOURNAL)
- `JOURNAL.md`는 **왜**를 담는다. 무엇을 했는지는 `git log`에 있으니 중복하지 않는다
- `ISSUES.md`의 핵심은 **교훈** 항목이다. 그게 없으면 그냥 에러 로그일 뿐이다
- 30분 이상 헤맨 문제는 해결했더라도 **반드시** `ISSUES.md`에 남긴다
...
- 환율을 아예 빼는 것 → 두 소스가 KRW를 못 주면 즉시 필요해진다. 폴백은 만들어둔다.
  수출입은행 API는 주말·공휴일에 데이터가 없으니 마지막 영업일 환율 폴백이 필수
- `coverage_pct` 없이 최저가만 보고 판단하는 것 → 커버리지 하락을 가격 상승으로 오독한다.
  차트도 규칙 엔진도 똑같이 속는다
- 알림에 신선도를 표시하지 않는 것 → Travelpayouts 캐시는 **최대 7일 묵는다**.
  238,000원 알림 보고 들어갔는데 265,000원이면 신뢰가 한 번에 무너진다

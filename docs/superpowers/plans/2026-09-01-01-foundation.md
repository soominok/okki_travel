...
**Tech Stack:** Python 3.12, FastAPI, SQLAlchemy 2.0(async, asyncpg), Alembic, pydantic-settings v2, structlog, pytest+pytest-asyncio, ruff, uv / Next.js 16 App Router (create-next-app@latest 기준), TypeScript strict, Tailwind v4
...
Expected: 전부 통과 (개수는 앞 태스크 누적에 따라 달라진다)
...
- [ ] `uv run pytest -v` → **전체 통과** (개수는 태스크가 쌓이며 늘어난다. 절대 숫자로 판정하지 말 것)
